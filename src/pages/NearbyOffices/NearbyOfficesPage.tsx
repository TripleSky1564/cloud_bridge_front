...
const regions: RegionOption[] = [
  {
    id: "gwangju-dong",
    label: "광주광역시 동구",
    center: { lat: 35.146, lng: 126.9235 },
  },
  {
    id: "gwangju-seo",
    label: "광주광역시 서구",
    center: { lat: 35.1522, lng: 126.8912 },
  },
  {
    id: "gwangju-nam",
    label: "광주광역시 남구",
    center: { lat: 35.1294, lng: 126.9027 },
  },
  {
    id: "gwangju-buk",
    label: "광주광역시 북구",
    center: { lat: 35.1743, lng: 126.9121 },
  },
  {
    id: "gwangju-gwangsan",
    label: "광주광역시 광산구",
    center: { lat: 35.1394, lng: 126.7931 },
  },
];

const NearbyOfficesPage = () => {
  const [mapError, setMapError] = useState<string | null>(null);
  const [selectedRegion, setSelectedRegion] = useState<RegionOption["id"]>(
    regions[0].id
  );
  ...
  const filteredOffices = useMemo(
    () =>
      offices.filter((office) =>
        office.regionCode ? office.regionCode === selectedRegion : true
      ),
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setFetchError(null);
    getJson<OfficeInfo[]>("/offices", { regionId: selectedRegion })
      .then((data) => {
        if (ignore) return;
        setOffices(data);
      })
    ...
    loadNaverMap()
      .then(() => {
        ...
        const center = new window.naver.maps.LatLng(
          regions[0].center.lat,
          regions[0].center.lng
        );
  ...
  useEffect(() => {
    ...
    const regionCenter = new naverMaps.LatLng(
    ...
    if (hasExtended) {
      const boundsWithGuard = bounds as typeof bounds & {
        isEmpty?: () => boolean;
      };
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.pageTitle}>
          <h1>가까운 관공서 찾기</h1>
          <p>
            내 주변의 복지·민원 기관을 지도에서 바로 확인하고, 연락처와
            운영시간도 함께 살펴보세요.
          </p>
        </div>
      ...
      <div className={styles.content}>
        <section className={styles.mapSection}>
          ...
          <p className={styles.mapHelper}>
            지도를 확대·축소하거나 마커를 눌러 상세 정보를 확인하세요.
          </p>
        </section>
        ...
        <section className={styles.listSection}>
          ...
          {!fetchError && !isLoading && filteredOffices.length === 0 ? (
            <div className={styles.emptyState}>
              <strong>
                {selectedRegionInfo.label}에서 등록된 관공서를 찾지 못했습니다.
              </strong>
            ...
            <div className={styles.cardsGrid}>
              {filteredOffices.map((office) => (
                <article key={office.id} className={styles.officeCard}>
                  ...
                  {office.notes && (
                    <p className={styles.meta}>{office.notes}</p>
                  )}
